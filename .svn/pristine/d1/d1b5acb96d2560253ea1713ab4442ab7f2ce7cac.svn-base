function varargout = Rename_Image(varargin)
% RENAME_IMAGE MATLAB code for Rename_Image.fig
%      RENAME_IMAGE, by itself, creates a new RENAME_IMAGE or raises the existing
%      singleton*.
%
%      H = RENAME_IMAGE returns the handle to a new RENAME_IMAGE or the handle to
%      the existing singleton*.
%
%      RENAME_IMAGE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in RENAME_IMAGE.M with the given input arguments.
%
%      RENAME_IMAGE('Property','Value',...) creates a new RENAME_IMAGE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Rename_Image_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Rename_Image_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Rename_Image

% Last Modified by GUIDE v2.5 01-Mar-2012 16:45:30

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Rename_Image_OpeningFcn, ...
                   'gui_OutputFcn',  @Rename_Image_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Rename_Image is made visible.
function Rename_Image_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Rename_Image (see VARARGIN)

% Choose default command line output for Rename_Image
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Rename_Image wait for user response (see UIRESUME)
...
if ~isempty(SpikeImageData)
    ...
    set(handles.OldName,'String',TextImage);
end
...
if (length(varargin)>1)
    ...
    set(handles.OldName,'Value',intersect(1:NumberImages,Settings.OldNameValue));  
...
function varargout = Rename_Image_OutputFcn(hObject, eventdata, handles) 
    ...
    if ~isempty(SpikeImageData)
        ...
        for i=1:length(SpikeImageData)
            if (ismember(i,ToChangeSelection) || SelectAll)
                if isempty(Rename)
                    SpikeImageData(i).Label.ListText=[Prefix,SpikeImageData(i).Label.ListText,Suffix];
                else
                    SpikeImageData(i).Label.ListText=[Prefix,Rename,Suffix];
                end
            end
        end
...
if ~isempty(SpikeImageData)
    ...
    for i=1:length(SpikeImageData)
        if (ismember(i,ToChangeSelection) || SelectAll)
            if isempty(Rename)
                NewNameList{i}=[num2str(i),' - ',Prefix,SpikeImageData(i).Label.ListText,Suffix];
            else
                NewNameList{i}=[num2str(i),' - ',Prefix,Rename,Suffix];
            end
        else
            NewNameList{i}=[num2str(i),' - ',SpikeImageData(i).Label.ListText];
        end
    end
