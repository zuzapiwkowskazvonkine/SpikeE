...
try
    ...
    Xpts_im=zeros(nb_pts_per_im,1);
    Ypts_im=zeros(nb_pts_per_im,1);
    ...
    for n=1:NumberImages
        ...
        prompt=['Give X and Y (space-separated) in new coord system for point ' int2str((n-1)*nb_pts_per_im+i) ': '];
        
        coord_str=input(prompt,'s');
        
        [xs,ys]=strread(coord_str,'%n%n','delimiter',' ');
        
        Xpts_new((n-1)*nb_pts_per_im+i)=xs;
        Ypts_new((n-1)*nb_pts_per_im+i)=ys;
