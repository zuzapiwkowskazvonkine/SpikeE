function varargout = Rename_Apps(varargin)
% RENAME_APPS MATLAB code for Rename_Apps.fig
%      RENAME_APPS, by itself, creates a new RENAME_APPS or raises the existing
%      singleton*.
%
%      H = RENAME_APPS returns the handle to a new RENAME_APPS or the handle to
%      the existing singleton*.
%
%      RENAME_APPS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in RENAME_APPS.M with the given input arguments.
%
%      RENAME_APPS('Property','Value',...) creates a new RENAME_APPS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Rename_Apps_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Rename_Apps_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Rename_Apps

% Last Modified by GUIDE v2.5 26-Feb-2012 00:25:43

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Rename_Apps_OpeningFcn, ...
                   'gui_OutputFcn',  @Rename_Apps_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Rename_Apps is made visible.
function Rename_Apps_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Rename_Apps (see VARARGIN)

% Choose default command line output for Rename_Apps
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Rename_Apps wait for user response (see UIRESUME)
...
if ~isempty(SpikeBatchData)
    for i=1:length(SpikeBatchData)
        TextApps{i}=[num2str(i),' - ',SpikeBatchData(i).Label.ListText];
    end
    set(handles.OldName,'String',TextApps);
end
...
if (length(varargin)>1)
    ...
    set(handles.OldName,'Value',intersect(1:NumberApps,Settings.OldNameValue));  
...
function varargout = Rename_Apps_OutputFcn(hObject, eventdata, handles) 
    ...
    if ~isempty(SpikeBatchData)
        ...
        for i=1:length(SpikeBatchData)
            if (ismember(i,ToChangeSelection) || SelectAll)
                if isempty(Rename)
                    SpikeBatchData(i).Label.ListText=[Prefix,SpikeBatchData(i).Label.ListText,Suffix];
                else
                    SpikeBatchData(i).Label.ListText=[Prefix,Rename,Suffix];
                end
            end
        end
...
if ~isempty(SpikeBatchData)
    ...
    for i=1:length(SpikeBatchData)
        if (ismember(i,ToChangeSelection) || SelectAll)
            if isempty(Rename)
                NewNameList{i}=[num2str(i),' - ',Prefix,SpikeBatchData(i).Label.ListText,Suffix];
            else
                NewNameList{i}=[num2str(i),' - ',Prefix,Rename,Suffix];
            end
        else
            NewNameList{i}=[num2str(i),' - ',SpikeBatchData(i).Label.ListText];
        end
    end
