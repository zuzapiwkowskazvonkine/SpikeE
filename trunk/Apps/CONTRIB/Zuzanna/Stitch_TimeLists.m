...
SpikeTraceData(BeginTrace).Path=SpikeTraceData(TracesToApply(1)).Path;
    ...
    binned_events=Digitize(SpikeTraceData(BeginTrace).XVector,binsize,firstbin,handles);
    
    SpikeTraceData(BeginTrace+1).XVector=firstbin:binsize:firstbin+binsize*(length(binned_events)-1);
    SpikeTraceData(BeginTrace+1).Trace=binned_events;
    SpikeTraceData(BeginTrace+1).DataSize=length(binned_events);
    
    name='binned events';
    SpikeTraceData(BeginTrace+1).Label.ListText=name;
    SpikeTraceData(BeginTrace+1).Label.YLabel='nb of events';
    SpikeTraceData(BeginTrace+1).Label.XLabel=SpikeTraceData(BeginTrace).Label.XLabel;
    SpikeTraceData(BeginTrace+1).Filename=SpikeTraceData(BeginTrace).Filename;
    SpikeTraceData(BeginTrace+1).Path=SpikeTraceData(BeginTrace).Path;
...
if ~isempty(SpikeTraceData)
    ...
    for k=1:group   
        for i=k:group:tot
            TextTrace{j}=[num2str(i),' - ',SpikeTraceData(i).Label.ListText];
            traces_LUT(j)=i;
            j=j+1;
        end
