function varargout = Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths(varargin)
% ANALYZE_PSTH_POISSON_DIFFBINS_BASELINES_FROM_PSTHS M-file for Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths.fig
%      ANALYZE_PSTH_POISSON_DIFFBINS_BASELINES_FROM_PSTHS, by itself, creates a new ANALYZE_PSTH_POISSON_DIFFBINS_BASELINES_FROM_PSTHS or raises the existing
%      singleton*.
%
%      H = ANALYZE_PSTH_POISSON_DIFFBINS_BASELINES_FROM_PSTHS returns the handle to a new ANALYZE_PSTH_POISSON_DIFFBINS_BASELINES_FROM_PSTHS or the handle to
%      the existing singleton*.
%
%      ANALYZE_PSTH_POISSON_DIFFBINS_BASELINES_FROM_PSTHS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ANALYZE_PSTH_POISSON_DIFFBINS_BASELINES_FROM_PSTHS.M with the given input arguments.
%
%      ANALYZE_PSTH_POISSON_DIFFBINS_BASELINES_FROM_PSTHS('Property','Value',...) creates a new ANALYZE_PSTH_POISSON_DIFFBINS_BASELINES_FROM_PSTHS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths

% Last Modified by GUIDE v2.5 29-Mar-2013 15:56:23

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths_OpeningFcn, ...
                   'gui_OutputFcn',  @Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths is made visible.
function Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths_OpeningFcn(hObject, ~, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths (see VARARGIN)

% Choose default command line output for Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths wait for user response (see UIRESUME)
% uiwait(handles.figure1);

global SpikeTraceData;

set(handles.SavePath,'String','C:\Users\Zuzanna\Documents\DataStanford2013\Analysis\Surprise_Purkinje');
...
if ~isempty(SpikeTraceData)
    ...
    set(handles.TraceNbPSTHs,'String',TextTrace);
...
function varargout = Analyze_PSTH_Poisson_diffbins_BaselineS_from_psths_OutputFcn(hObject, eventdata, handles) 
...
Settings.TraceNbPSTHsString=get(handles.TraceNbPSTHs,'String');
Settings.TraceNbPSTHsValue=get(handles.TraceNbPSTHs,'Value');
...
TraceNbPSTHs=get(handles.TraceNbPSTHs,'Value');
...
baseline=zeros(1,length(TracesToApply)); % we want one value per PSTH to analyze (but PSTHs originating from same File/same param. set will have the same value)

tottraces=0;
for z=1:length(SpikeTraceData(TraceNbPSTHs).Trace) %loop over all Files/PSTH "sets"
    
    firstpsth=tottraces+1;
    nbtraces=SpikeTraceData(TraceNbPSTHs).Trace(z);
    tottraces=tottraces+nbtraces;
    
    bsavg=zeros(1,nbtraces);
    bsavghz=zeros(1,nbtraces);
    i=1;
    
    psth_inds=TracesToApply(firstpsth):TracesToApply(firstpsth+nbtraces-1)
    
    for k=psth_inds         %loop over all PSTHs in one File/"set" 
    ...
    baseline(psth_inds-TracesToApply(1)+1)=mean(bsavghz);
    
end

bsavghzall=mean(baseline);
set(handles.BaselineAvg,'String',num2str(bsavghzall));

BeginTrace=length(SpikeTraceData)+1;
SpikeTraceData(BeginTrace).XVector=1:length(baseline); %%%%
SpikeTraceData(BeginTrace).Trace=baseline;
SpikeTraceData(BeginTrace).DataSize=length(baseline);

name=['baseline avg rate, bins ' num2str(startbin) '-' num2str(stopbin) ', from PSTHs pooled per File'];
...
handles.BaselineSaveIndex=BeginTrace; % store index of SpikeTraceData here for future use and future saving of info
guidata(hObject,handles);
...
bs_ind=handles.BaselineSaveIndex; % index of Trace containing baseline rates
...
for b=binsize1:binsizestep:binsize2 % loop over binsizes
    ...
    for s=1:binsize  %loop over all possible starting positions for the first bin
        ...
        for k=TracesToApply
            ...
            bsmeanrate=SpikeTraceData(bs_ind).Trace(psth_order);
...
% --- Executes on selection change in TraceNbPSTHs.
function TraceNbPSTHs_Callback(hObject, eventdata, handles)
% hObject    handle to TraceNbPSTHs (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns TraceNbPSTHs contents as cell array
%        contents{get(hObject,'Value')} returns selected item from TraceNbPSTHs


% --- Executes during object creation, after setting all properties.
function TraceNbPSTHs_CreateFcn(hObject, eventdata, handles)
% hObject    handle to TraceNbPSTHs (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
