function varargout = Detect_Spikes_LK_home1(varargin)
% DETECT_SPIKES_LK_HOME1 M-file for Detect_Spikes_LK_home1.fig
%      DETECT_SPIKES_LK_HOME1, by itself, creates a new DETECT_SPIKES_LK_HOME1 or raises the existing
%      singleton*.
%
%      H = DETECT_SPIKES_LK_HOME1 returns the handle to a new DETECT_SPIKES_LK_HOME1 or the handle to
%      the existing singleton*.
%
%      DETECT_SPIKES_LK_HOME1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DETECT_SPIKES_LK_HOME1.M with the given input arguments.
%
%      DETECT_SPIKES_LK_HOME1('Property','Value',...) creates a new DETECT_SPIKES_LK_HOME1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Detect_Spikes_LK_home1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Detect_Spikes_LK_home1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Detect_Spikes_LK_home1

% Last Modified by GUIDE v2.5 19-Oct-2012 22:17:26

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Detect_Spikes_LK_home1_OpeningFcn, ...
                   'gui_OutputFcn',  @Detect_Spikes_LK_home1_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Detect_Spikes_LK_home1 is made visible.
function Detect_Spikes_LK_home1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Detect_Spikes_LK_home1 (see VARARGIN)

% Choose default command line output for Detect_Spikes_LK_home1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Detect_Spikes_LK_home1 wait for user response (see UIRESUME)
...
if (length(varargin)>1)
    ...
    set(handles.NumClusters, 'String', Settings.NumClustersString);
    set(handles.NumPCsToCluster, 'String', Settings.NumPCsToClusterString);
    set(handles.SaveSnippets, 'Value', Settings.SaveSnippetsValue);
    ...
    set(handles.SecondTry,'Value',Settings.SecondTryValue);
    set(handles.RangeThresh,'String',Settings.RangeThreshString);
    
   
    
    handles.rangeData=Settings.rangeData;
...
function varargout = Detect_Spikes_LK_home1_OutputFcn(hObject, eventdata, handles) 
...
Settings.SecondTryValue=get(handles.SecondTry,'Value');
Settings.RangeThreshString=get(handles.RangeThresh,'String');

% MODIF AT HOME:
Settings.rangeData=handles.rangeData;
...
rangethresh=str2double(get(handles.RangeThresh,'String'));
secondtry=get(handles.SecondTry,'Value');
...
% second round: gather snippets of trace for portions of trace where range is above RangeThresh     

rangedata = handles.rangeData;
figure;
plot(rangedata);
aboverangethresh=find(rangedata>rangethresh); %indices of rangeData for which value is above chosen threshold
% gather snippets corresponding to those indices only 
snipInd=0;
lastCind=0;
% snippets=zeros(fs*msBefore+fs*msAfter+1, length(crossInds));
snippets=zeros(length(aboverangethresh), length(crossInds));
...
for c=1:length(crossInds)
    ...
    if (cInd>fs*msBefore) && (cInd-lastCind)>fs*msBetween && cInd+fs*msAfter<=length(trace) % && abs(trace(cInd+minpts))>abs(thresh)
        snipInd=snipInd+1;
        thisSnippet=trace(cInd-fs*msBefore:cInd+fs*msAfter);
        snippetsplot(:,snipInd)=thisSnippet;
        thisSnippet=thisSnippet(aboverangethresh);
        snippets(:,snipInd)=thisSnippet;
...
mintraces=min(Data);
maxtraces=max(Data);
rangedata1=maxtraces-mintraces;
handles.rangeData=rangedata1;
guidata(hObject,handles);
else
snippetsplot=snippetsplot(:, 1:snipInd);
end
...
for k=1:numClusts
    ...
    if secondtry==1
    snipsByClustplot{k}=snippetsplot(:,idx==k);
    end
    figure(nf+1); nf=nf+1; 
    if secondtry==0
    plot(snipsByClust{k})
    hold on
    plot(rangedata1,'k','LineWidth',8);
    else
    plot(snipsByClustplot{k})
    end
...
SpikeTraceData(traceInd).Label.ListText=['c', num2str(k), ' only times (ZP)'];
SpikeTraceData(traceInd).Label.YLabel='detections';
SpikeTraceData(traceInd).Label.XLabel=SpikeTraceData(ephysTrace).Label.XLabel;
SpikeTraceData(traceInd).Filename=SpikeTraceData(ephysTrace).Filename;
SpikeTraceData(traceInd).Path=SpikeTraceData(ephysTrace).Path;
...
if get(handles.SaveSnippets, 'Value')
    ...
    for k=1:numClusts
        ...
        for sInd=1:size(snipsByClust{k}, 2)
            ...
            SpikeTraceData(traceInd).DataSize=length(snipsByClust{k}(:,sInd));
            ...
            SpikeTraceData(traceInd).Label.YLabel=SpikeTraceData(ephysTrace).Label.YLabel;
            SpikeTraceData(traceInd).Label.XLabel=SpikeTraceData(ephysTrace).Label.XLabel;
            SpikeTraceData(traceInd).Filename=SpikeTraceData(ephysTrace).Filename;
            SpikeTraceData(traceInd).Path=SpikeTraceData(ephysTrace).Path;
        end
...
% --- Executes on button press in SecondTry.
function SecondTry_Callback(hObject, eventdata, handles)
% hObject    handle to SecondTry (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of SecondTry



function RangeThresh_Callback(hObject, eventdata, handles)
% hObject    handle to RangeThresh (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of RangeThresh as text
%        str2double(get(hObject,'String')) returns contents of RangeThresh as a double


% --- Executes during object creation, after setting all properties.
function RangeThresh_CreateFcn(hObject, eventdata, handles)
% hObject    handle to RangeThresh (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
