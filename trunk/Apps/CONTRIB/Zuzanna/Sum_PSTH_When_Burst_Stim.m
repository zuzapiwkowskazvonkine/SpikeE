function varargout = Sum_PSTH_When_Burst_Stim(varargin)
% SUM_PSTH_WHEN_BURST_STIM M-file for Sum_PSTH_When_Burst_Stim.fig
%      SUM_PSTH_WHEN_BURST_STIM, by itself, creates a new SUM_PSTH_WHEN_BURST_STIM or raises the existing
%      singleton*.
%
%      H = SUM_PSTH_WHEN_BURST_STIM returns the handle to a new SUM_PSTH_WHEN_BURST_STIM or the handle to
%      the existing singleton*.
%
%      SUM_PSTH_WHEN_BURST_STIM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SUM_PSTH_WHEN_BURST_STIM.M with the given input arguments.
%
%      SUM_PSTH_WHEN_BURST_STIM('Property','Value',...) creates a new SUM_PSTH_WHEN_BURST_STIM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Sum_PSTH_When_Burst_Stim_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Sum_PSTH_When_Burst_Stim_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Sum_PSTH_When_Burst_Stim

% Last Modified by GUIDE v2.5 17-Dec-2012 15:36:06

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Sum_PSTH_When_Burst_Stim_OpeningFcn, ...
                   'gui_OutputFcn',  @Sum_PSTH_When_Burst_Stim_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Sum_PSTH_When_Burst_Stim is made visible.
function Sum_PSTH_When_Burst_Stim_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Sum_PSTH_When_Burst_Stim (see VARARGIN)

% Choose default command line output for Sum_PSTH_When_Burst_Stim
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Sum_PSTH_When_Burst_Stim wait for user response (see UIRESUME)
...
%     set(handles.TraceSelectorOld,'Value',Settings.TraceSelectorValue);
    set(handles.BurstPeriod,'String',Settings.BurstPeriodString);
...
function varargout = Sum_PSTH_When_Burst_Stim_OutputFcn(hObject, eventdata, handles) 
