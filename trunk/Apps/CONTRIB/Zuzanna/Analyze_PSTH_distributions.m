function varargout = Analyze_PSTH_distributions(varargin)
% ANALYZE_PSTH_DISTRIBUTIONS M-file for Analyze_PSTH_distributions.fig
%      ANALYZE_PSTH_DISTRIBUTIONS, by itself, creates a new ANALYZE_PSTH_DISTRIBUTIONS or raises the existing
%      singleton*.
%
%      H = ANALYZE_PSTH_DISTRIBUTIONS returns the handle to a new ANALYZE_PSTH_DISTRIBUTIONS or the handle to
%      the existing singleton*.
%
%      ANALYZE_PSTH_DISTRIBUTIONS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ANALYZE_PSTH_DISTRIBUTIONS.M with the given input arguments.
%
%      ANALYZE_PSTH_DISTRIBUTIONS('Property','Value',...) creates a new ANALYZE_PSTH_DISTRIBUTIONS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Analyze_PSTH_distributions_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Analyze_PSTH_distributions_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Analyze_PSTH_distributions

% Last Modified by GUIDE v2.5 26-Oct-2012 16:02:05

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Analyze_PSTH_distributions_OpeningFcn, ...
                   'gui_OutputFcn',  @Analyze_PSTH_distributions_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Analyze_PSTH_distributions is made visible.
function Analyze_PSTH_distributions_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Analyze_PSTH_distributions (see VARARGIN)

% Choose default command line output for Analyze_PSTH_distributions
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Analyze_PSTH_distributions wait for user response (see UIRESUME)
...
function varargout = Analyze_PSTH_distributions_OutputFcn(hObject, eventdata, handles) 
...
for k=TracesToApply
    ...
    figure(fign);
    bins=[10:50:1000];
    plot(hist(SpikeTraceData(k).Trace(startbin-firstbin+1:stopbin-firstbin),bins));
