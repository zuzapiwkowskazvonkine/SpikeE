function varargout = Analyze_PSTH_Poisson_diffbins_LoadedBaselineS(varargin)
% ANALYZE_PSTH_POISSON_DIFFBINS_LOADEDBASELINES M-file for Analyze_PSTH_Poisson_diffbins_LoadedBaselineS.fig
%      ANALYZE_PSTH_POISSON_DIFFBINS_LOADEDBASELINES, by itself, creates a new ANALYZE_PSTH_POISSON_DIFFBINS_LOADEDBASELINES or raises the existing
%      singleton*.
%
%      H = ANALYZE_PSTH_POISSON_DIFFBINS_LOADEDBASELINES returns the handle to a new ANALYZE_PSTH_POISSON_DIFFBINS_LOADEDBASELINES or the handle to
%      the existing singleton*.
%
%      ANALYZE_PSTH_POISSON_DIFFBINS_LOADEDBASELINES('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ANALYZE_PSTH_POISSON_DIFFBINS_LOADEDBASELINES.M with the given input arguments.
%
%      ANALYZE_PSTH_POISSON_DIFFBINS_LOADEDBASELINES('Property','Value',...) creates a new ANALYZE_PSTH_POISSON_DIFFBINS_LOADEDBASELINES or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Analyze_PSTH_Poisson_diffbins_LoadedBaselineS_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Analyze_PSTH_Poisson_diffbins_LoadedBaselineS_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Analyze_PSTH_Poisson_diffbins_LoadedBaselineS

% Last Modified by GUIDE v2.5 12-Mar-2013 11:08:03

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Analyze_PSTH_Poisson_diffbins_LoadedBaselineS_OpeningFcn, ...
                   'gui_OutputFcn',  @Analyze_PSTH_Poisson_diffbins_LoadedBaselineS_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Analyze_PSTH_Poisson_diffbins_LoadedBaselineS is made visible.
function Analyze_PSTH_Poisson_diffbins_LoadedBaselineS_OpeningFcn(hObject, ~, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Analyze_PSTH_Poisson_diffbins_LoadedBaselineS (see VARARGIN)

% Choose default command line output for Analyze_PSTH_Poisson_diffbins_LoadedBaselineS
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Analyze_PSTH_Poisson_diffbins_LoadedBaselineS wait for user response (see UIRESUME)
...
if ~isempty(SpikeTraceData)
    ...
    set(handles.TraceSelectorBaseline,'String',TextTrace);
...
function varargout = Analyze_PSTH_Poisson_diffbins_LoadedBaselineS_OutputFcn(hObject, eventdata, handles) 
...
Settings.TraceSelectorBaselineString=get(handles.TraceSelectorBaseline,'String');
Settings.TraceSelectorBaselineValue=get(handles.TraceSelectorBaseline,'Value');
...
TraceBaseline=get(handles.TraceSelectorBaseline,'Value');
TracePSTHs=get(handles.TraceSelector,'Value');

bsavghzall=mean(SpikeTraceData(TraceBaseline).Trace);
...
name=['baseline avg rate, from loaded vector'];
...
SpikeTraceData(BeginTrace).Filename=SpikeTraceData(TraceBaseline).Filename;
SpikeTraceData(BeginTrace).Path=SpikeTraceData(TraceBaseline).Path;

%%

baselinename=SpikeTraceData(TraceBaseline).Label.ListText;
pind=strfind(baselinename,'C');
baselinepath=baselinename(min(pind):end)

totbase=length(SpikeTraceData(TraceBaseline).Trace);

% global baseepstart
% global baseepend
% global basefilenb

baseepstart=zeros(1,totbase);
baseepend=zeros(1,totbase);
basefilenb=zeros(1,totbase);

n=1;
listfiles=dir(baselinepath);
for k=1:length(listfiles)
   name=listfiles(k).name;
   bsind=strfind(name,'baseline');
   if ~isempty(bsind)
       basefilenb(n)=sscanf(name,'%*12c %d %*s');
       epind=strfind(name,'ep');
       name=name(epind+2:bsind-1);
       baseepstart(n)=sscanf(name,'%d %*s');
       ifind=strfind(name,'-');
       baseepend(n)=sscanf(name(ifind+1:end),'%d');
   n=n+1;
   end
    
end

baselines=zeros(1,length(TracePSTHs));

%Ep Start and Ep End Traces are the 2 Traces just after the PSTH Traces,
%based on the way they are all loaded by Load_Multiple_Mat_Files:
EpStartTrace=TracePSTHs(end)+1;
EpEndTrace=EpStartTrace+1;

n=1;
for k=TracePSTHs
   epstart=SpikeTraceData(EpStartTrace).Trace(n);
   epend=SpikeTraceData(EpEndTrace).Trace(n);
   
   baseind1=find(baseepstart==epstart);
   baseind2=find(baseepend==epend);
   
   baseind=intersect(baseind1,baseind2);
   
   if length(baseind)>1  %ie more than 1 set of epstart-epend fitting with the psth is found
       name=SpikeTraceData(k).Filename;
       filenb_psth=sscanf(name,'%*12c %d %*s');
       baseind3=find(basefilenb==filenb_psth);
       baseind=intersect(baseind,baseind3); 
   end
   
   baselines(n)=SpikeTraceData(TraceBaseline).Trace(baseind);
    
    n=n+1;
end

BeginTrace=length(SpikeTraceData)+1;
SpikeTraceData(BeginTrace).XVector=1:length(baselines); %%%%
SpikeTraceData(BeginTrace).Trace=baselines;
SpikeTraceData(BeginTrace).DataSize=length(baselines);

name=['baselines for each PSTH, from loaded vector'];
SpikeTraceData(BeginTrace).Label.ListText=name;
SpikeTraceData(BeginTrace).Label.YLabel='Hz';
SpikeTraceData(BeginTrace).Label.XLabel='';
SpikeTraceData(BeginTrace).Filename=SpikeTraceData(TraceBaseline).Filename;
...
handles.BaselineSaveIndex=[BeginTrace-1 BeginTrace]; % store index of SpikeTraceData here for future saving of info
guidata(hObject,handles);
...
for b=binsize1:binsizestep:binsize2 % loop over binsizes
    ...
    for s=1:binsize  %loop over all possible starting positions for the first bin
        ...
        for k=TracesToApply
            ...
            % get baseline rate from the baselines vector obtained
            % previously, which index in Traces is stored in handles.BaselineSaveIndex:
            k
            bsmeanrate=SpikeTraceData(handles.BaselineSaveIndex(2)).Trace(k-TracesToApply(1)+1)
            ...
            for z=1:length(PSTH)
                ...
                if m>PoissonExpspikenb
                    ...
                    if tempsurprise<10^(-15)   %double precision limitation
                        surprise(z)=-15;
                    else
...
% --- Executes on selection change in TraceSelectorBaseline.
function TraceSelectorBaseline_Callback(hObject, eventdata, handles)
% hObject    handle to TraceSelectorBaseline (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns TraceSelectorBaseline contents as cell array
%        contents{get(hObject,'Value')} returns selected item from TraceSelectorBaseline


% --- Executes during object creation, after setting all properties.
function TraceSelectorBaseline_CreateFcn(hObject, eventdata, handles)
% hObject    handle to TraceSelectorBaseline (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
