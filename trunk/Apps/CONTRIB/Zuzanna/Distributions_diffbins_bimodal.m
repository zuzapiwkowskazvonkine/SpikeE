function varargout = Distributions_diffbins_bimodal(varargin)
% DISTRIBUTIONS_DIFFBINS_BIMODAL M-file for Distributions_diffbins_bimodal.fig
%      DISTRIBUTIONS_DIFFBINS_BIMODAL, by itself, creates a new DISTRIBUTIONS_DIFFBINS_BIMODAL or raises the existing
%      singleton*.
%
%      H = DISTRIBUTIONS_DIFFBINS_BIMODAL returns the handle to a new DISTRIBUTIONS_DIFFBINS_BIMODAL or the handle to
%      the existing singleton*.
%
%      DISTRIBUTIONS_DIFFBINS_BIMODAL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DISTRIBUTIONS_DIFFBINS_BIMODAL.M with the given input arguments.
%
%      DISTRIBUTIONS_DIFFBINS_BIMODAL('Property','Value',...) creates a new DISTRIBUTIONS_DIFFBINS_BIMODAL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Distributions_diffbins_bimodal_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Distributions_diffbins_bimodal_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Distributions_diffbins_bimodal

% Last Modified by GUIDE v2.5 12-Mar-2013 15:49:47

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Distributions_diffbins_bimodal_OpeningFcn, ...
                   'gui_OutputFcn',  @Distributions_diffbins_bimodal_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Distributions_diffbins_bimodal is made visible.
function Distributions_diffbins_bimodal_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Distributions_diffbins_bimodal (see VARARGIN)

% Choose default command line output for Distributions_diffbins_bimodal
handles.output = hObject;

% UIWAIT makes Distributions_diffbins_bimodal wait for user response (see UIRESUME)
% uiwait(handles.figure1);

global SpikeTraceData;

    set(handles.PathForLoading,'String','C:\Users\Zuzanna\Documents\DataStanford2008-2012\ephys_data\Surprise_Ai35_DCN');
    handles.Path='C:\Users\Zuzanna\Documents\DataStanford2008-2012\ephys_data\Surprise_Ai35_DCN';
...
function varargout = Distributions_diffbins_bimodal_OutputFcn(hObject, eventdata, handles) 
...
getthresh=1; % get(handles.GetThresh,'Value');
...
for i=3:nbdirs %loop over folders (first 2 are '.' and '..')
    ...
    if (strcmp(nametoparse,'PSTHs') | strcmp(nametoparse,'Analysis_Windows') | strcmp(nametoparse,'Other'))
        ...
        binnb=floor(max(temp1pos));
        [hy1pos,hxpos]=hist(temp1pos,[0:binnb]);
        ...
        if negsurprise
            ...
            binnb=ceil(min(temp1neg));
            [hy1neg,hxneg]=hist(temp1neg,[binnb:0]);
        ...
        if getthresh
            ...
            % find indices of the two modes by assuming that each mode is
            % the max in one half of the histogram:
            half=floor(length(hy1pos)/2);
            [y1,ind1]=max(hy1pos(1:half));
            [y2,ind2]=max(hy1pos(half+1:end));
            ind1=min(ind1);
            ind2=min(ind2)+half;
            
            %             if length(inds)>1
            [val,threshind]=min(hy1pos(ind1:ind2)); %get minimum between the two local maxima of the bimodal distrib
            threshinds=find(hy1pos(ind1:ind2)==val); % do this in case there are several bins with min value (choose bin closest to 0 below)
            thresh=hxpos(threshinds(1)+ind1-1);
            %             else
            %                 falseposratio=0.01;
            ...
            if negsurprise
                ...
                half=floor(length(hy1neg)/2);
                [y1,ind1]=max(hy1neg(1:half));
                [y2,ind2]=max(hy1neg(half+1:end));
                ind1=min(ind1);
                ind2=min(ind2)+half;
                %                 if length(inds)>1
                [val,threshind]=min(hy1neg((ind1:ind2))); %get minimum between the two local maxima of the bimodal distrib
                threshinds=find(hy1neg(ind1:ind2)==val); % do this in case there are several bins with min value (choose bin closest to 0 below)
                threshneg=hxneg(threshinds(end)+ind1-1);
                %                 else
                %                     falseposratio=0.01;
        ...
        if maxsurprise
            name=['surprise thresholds, from bimodal distrib of max bins of PSTHs'];
        else
            name=['surprise thresholds, from bimodal distrib of all bins of PSTHs' ];
        end
