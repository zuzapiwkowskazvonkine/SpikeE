function varargout = SignificantBins_diffbins_ChooseThresh(varargin)
% SIGNIFICANTBINS_DIFFBINS_CHOOSETHRESH M-file for SignificantBins_diffbins_ChooseThresh.fig
%      SIGNIFICANTBINS_DIFFBINS_CHOOSETHRESH, by itself, creates a new SIGNIFICANTBINS_DIFFBINS_CHOOSETHRESH or raises the existing
%      singleton*.
%
%      H = SIGNIFICANTBINS_DIFFBINS_CHOOSETHRESH returns the handle to a new SIGNIFICANTBINS_DIFFBINS_CHOOSETHRESH or the handle to
%      the existing singleton*.
%
%      SIGNIFICANTBINS_DIFFBINS_CHOOSETHRESH('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SIGNIFICANTBINS_DIFFBINS_CHOOSETHRESH.M with the given input arguments.
%
%      SIGNIFICANTBINS_DIFFBINS_CHOOSETHRESH('Property','Value',...) creates a new SIGNIFICANTBINS_DIFFBINS_CHOOSETHRESH or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SignificantBins_diffbins_ChooseThresh_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SignificantBins_diffbins_ChooseThresh_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SignificantBins_diffbins_ChooseThresh

% Last Modified by GUIDE v2.5 07-Mar-2013 16:31:53

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SignificantBins_diffbins_ChooseThresh_OpeningFcn, ...
                   'gui_OutputFcn',  @SignificantBins_diffbins_ChooseThresh_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SignificantBins_diffbins_ChooseThresh is made visible.
function SignificantBins_diffbins_ChooseThresh_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SignificantBins_diffbins_ChooseThresh (see VARARGIN)

% Choose default command line output for SignificantBins_diffbins_ChooseThresh
handles.output = hObject;



% UIWAIT makes SignificantBins_diffbins_ChooseThresh wait for user response (see UIRESUME)
% uiwait(handles.figure1);

global SpikeTraceData;

    set(handles.PathForLoading,'String','C:\Users\Zuzanna\Documents\DataStanford2008-2012\ephys_data\Surprise_Ai35_Purkinje');
    handles.Path='C:\Users\Zuzanna\Documents\DataStanford2008-2012\ephys_data\Surprise_Ai35_Purkinje';
    
    set(handles.PathForLoadingThresh,'String','C:\Users\Zuzanna\Documents\DataStanford2008-2012\ephys_data\Surprise_Spont_Ai35_Purkinje');
    handles.Path2='C:\Users\Zuzanna\Documents\DataStanford2008-2012\ephys_data\Surprise_Spont_Ai35_Purkinje';
...
if (length(varargin)>1)
    ...
    set(handles.PathForLoadingThresh,'String',Settings.PathForLoadingThreshString);
...
function varargout = SignificantBins_diffbins_ChooseThresh_OutputFcn(hObject, eventdata, handles) 
...
Settings.PathForLoadingThreshString=get(handles.PathForLoadingThresh,'String');
...
for i=3:nbdirs %loop over folders (first 2 are '.' and '..')
    ...
    if (strcmp(nametoparse,'PSTHs') | strcmp(nametoparse,'Analysis_Windows') | strcmp(nametoparse,'Other'))
        ...
        for j=3:nbfiles %loop over files in the corresponding folder on Path2 to get threshold values
            
            nameend=allfiles(j).name(end-7:end-4);
            if strcmp(nameend,'info')
                
                % download threshold(s) trace
                LocalFile=[handles.Path2 '\' allfolders(i).name '\' allfiles(j).name]; %% note that Path2 is used to get the Thresholds now
        ...
        for j=3:nbfiles %loop over files in this folder (on Path) to load surprise PSTHs
            nameend=allfiles(j).name(end-7:end-4);
...
for n=1:length(cells)
    ...
    if possurprise
        nbconds=nbdirs-5; % CAREFUL! remove 5 because 3 specific folders also present already (PSTHs, Other, Analysis_Windows); create them first before running this App
...
%         stoptraces=length(SpikeTraceData);  % last converted trace to analyze below
        ...
        %window containing all the significant bins from all PSTHs (starting at 0 or later)
        ...
        for k=starttot:endtot
            ...
            vec=SpikeTraceData(k).Trace(addind+1:end); %start analysis at stimulus time (ie 0).
            signbins=find(vec==1);
...
%         stoptraces=length(SpikeTraceData);  % last converted trace to analyze below
        ...
        for k=starttraces:starttraces+tracesperbinsize-1  % loop over PSTHs
            ...
            for j=1:nbconds
                ...
                if k==starttraces
                    minshow=minstart
                tracenb=k+tracesperbinsize*(j-1)
                end
        ...
        %define common response analysis window for all PSTHs, ie largest
        %window containing all the significant bins from all PSTHs (startong at 0 or later)
        startanalysis=length(SpikeTraceData(starttot).XVector); 
        stopanalysis=0;
...
% --- Executes on button press in SetPathThresh.
function SetPathThresh_Callback(hObject, eventdata, handles)
% hObject    handle to SetPathThresh (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if isequal(NewPath,0)
    ...
    set(handles.PathForLoadingThresh,'String',NewPath);
    guidata(hObject,handles);
%     CreateFileName(hObject, handles);
end
