function varargout = Make_Grid_of_Responses_Merged_StimDur(varargin)
% MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR M-file for Make_Grid_of_Responses_Merged_StimDur.fig
%      MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR, by itself, creates a new MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR or raises the existing
%      singleton*.
%
%      H = MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR returns the handle to a new MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR or the handle to
%      the existing singleton*.
%
%      MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR.M with the given input arguments.
%
%      MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR('Property','Value',...) creates a new MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Make_Grid_of_Responses_Merged_StimDur_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Make_Grid_of_Responses_Merged_StimDur_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Make_Grid_of_Responses_Merged_StimDur

% Last Modified by GUIDE v2.5 14-Mar-2013 15:01:53

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Make_Grid_of_Responses_Merged_StimDur_OpeningFcn, ...
                   'gui_OutputFcn',  @Make_Grid_of_Responses_Merged_StimDur_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Make_Grid_of_Responses_Merged_StimDur is made visible.
function Make_Grid_of_Responses_Merged_StimDur_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Make_Grid_of_Responses_Merged_StimDur (see VARARGIN)

% Choose default command line output for Make_Grid_of_Responses_Merged_StimDur
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Make_Grid_of_Responses_Merged_StimDur wait for user response (see UIRESUME)
...
function varargout = Make_Grid_of_Responses_Merged_StimDur_OutputFcn(hObject, eventdata, handles) 
...
for z=1:length(SpikeTraceData(nbtracestrace).Trace) % loop over Files (ie groups of PSTHs from one stimulus sequence)    
    ...
    if SpikeTraceData(stimdurstrace).Trace(k)==stimdurtarget  
        lastfile=z;
    end
end
