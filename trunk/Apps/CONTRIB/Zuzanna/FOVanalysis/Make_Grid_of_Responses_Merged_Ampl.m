function varargout = Make_Grid_of_Responses_Merged(varargin)
% MAKE_GRID_OF_RESPONSES_MERGED M-file for Make_Grid_of_Responses_Merged.fig
%      MAKE_GRID_OF_RESPONSES_MERGED, by itself, creates a new MAKE_GRID_OF_RESPONSES_MERGED or raises the existing
%      singleton*.
%
%      H = MAKE_GRID_OF_RESPONSES_MERGED returns the handle to a new MAKE_GRID_OF_RESPONSES_MERGED or the handle to
%      the existing singleton*.
%
%      MAKE_GRID_OF_RESPONSES_MERGED('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MAKE_GRID_OF_RESPONSES_MERGED.M with the given input arguments.
%
%      MAKE_GRID_OF_RESPONSES_MERGED('Property','Value',...) creates a new MAKE_GRID_OF_RESPONSES_MERGED or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Make_Grid_of_Responses_Merged_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Make_Grid_of_Responses_Merged_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Make_Grid_of_Responses_Merged

% Last Modified by GUIDE v2.5 26-Feb-2013 14:50:40

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Make_Grid_of_Responses_Merged_OpeningFcn, ...
                   'gui_OutputFcn',  @Make_Grid_of_Responses_Merged_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Make_Grid_of_Responses_Merged is made visible.
function Make_Grid_of_Responses_Merged_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Make_Grid_of_Responses_Merged (see VARARGIN)

% Choose default command line output for Make_Grid_of_Responses_Merged
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Make_Grid_of_Responses_Merged wait for user response (see UIRESUME)
...
set(handles.SavingPath,'String','C:\Users\Zuzanna\Documents\DataStanford2013\Analysis\Surprise_DCN\Other');
handles.Path='C:\Users\Zuzanna\Documents\DataStanford2013\Analysis\Surprise_DCN\Other';
...
function varargout = Make_Grid_of_Responses_Merged_OutputFcn(hObject, eventdata, handles) 
...
for z=1:length(SpikeTraceData(nbtracestrace).Trace) % loop over Files (ie groups of PSTHs from one stimulus sequence)
    ...
    ys=[];
        ...
        if int==oldint
            ...
            if z==length(SpikeTraceData(nbtracestrace).Trace)  % last File -> plot and save
                ...
                for n=1:length(Coords2p_Array)
                    ...
                    if CoordsSign2p_Array{n}~=0
                        plot_rect_on_image_shift(CoordsSign2p_Array{n},'r',p,xstart,ystart);  %%%%% NEED TO CHANGE COLOR HERE
                    end
                    
                end
                
                % save fig p
                fname=[handles.Path '\' 'Int' num2str(int) '_' num2str(intW) 'uW_' SpikeTraceData(inds(1)).Filename '.fig'];
                title(['Int ' num2str(int) '/' num2str(intW) ' uW']);
                saveas(p,fname);
            end
            ...
            if oldint>0 % this condition excludes the first File
                ...
                % save fig p
                fname=[handles.Path '\' 'Int' num2str(oldint) '_' num2str(oldintW) 'uW_' SpikeTraceData(oldinds(1)).Filename '.fig'];
                title(['Int ' num2str(oldint) '/' num2str(oldintW) ' uW']);
                saveas(p,fname);
            end
...
%make thick red lines   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CHANGE COLOR HERE
if strcmp(scolor,'r')
    lw=2;
else
    lw=0.5;
...
for k=1:size(coords,2)
    plot([coords(2,k) coords(4,k)],[coords(1,k) coords(3,k)],scolor,'LineWidth',lw); % line between TL and TR corners (again, plot X as a function of Y and not the opposite)
    hold on
    plot([coords(8,k) coords(4,k)],[coords(7,k) coords(3,k)],scolor,'LineWidth',lw); % BR -> TR corners
    hold on
    plot([coords(6,k) coords(8,k)],[coords(5,k) coords(7,k)],scolor,'LineWidth',lw); % BL-> BR corners
    hold on
    plot([coords(6,k) coords(2,k)],[coords(5,k) coords(1,k)],scolor,'LineWidth',lw); % BL -> TL corners
    hold on  
end
