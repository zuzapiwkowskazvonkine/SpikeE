function varargout = Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved(varargin)
% MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR_FLAGGEDREMOVED M-file for Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved.fig
%      MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR_FLAGGEDREMOVED, by itself, creates a new MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR_FLAGGEDREMOVED or raises the existing
%      singleton*.
%
%      H = MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR_FLAGGEDREMOVED returns the handle to a new MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR_FLAGGEDREMOVED or the handle to
%      the existing singleton*.
%
%      MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR_FLAGGEDREMOVED('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR_FLAGGEDREMOVED.M with the given input arguments.
%
%      MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR_FLAGGEDREMOVED('Property','Value',...) creates a new MAKE_GRID_OF_RESPONSES_MERGED_STIMDUR_FLAGGEDREMOVED or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved

% Last Modified by GUIDE v2.5 16-Mar-2013 12:12:51

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved_OpeningFcn, ...
                   'gui_OutputFcn',  @Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved is made visible.
function Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved (see VARARGIN)

% Choose default command line output for Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved wait for user response (see UIRESUME)
...
handles.Path='C:\Users\Zuzanna\Documents\DataStanford2008-2012\ephys_data\Surprise_Ai27_DCN\Other';
   ...
   if ~isempty(SpikeTraceData)
       ...
       set(handles.TraceStimDurs,'String',TextTrace);
       set(handles.FlaggedTrace,'String',TextTrace);
   end
...
if (length(varargin)>1)
    ...
    set(handles.TraceStimDurs,'String',Settings.TraceStimDursString);
    set(handles.TraceStimDurs,'Value',Settings.TraceStimDursValue);
    set(handles.StimDurTarget,'String',Settings.StimDurTargetString);
    
    set(handles.FlaggedTrace,'String',Settings.FlaggedTraceString);
    set(handles.FlaggedTrace,'Value',Settings.FlaggedTraceValue);
...
function varargout = Make_Grid_of_Responses_Merged_StimDur_FlaggedRemoved_OutputFcn(hObject, eventdata, handles) 
...
Settings.StimDurTargetString=get(handles.StimDurTarget,'String');

Settings.FlaggedTraceString=get(handles.FlaggedTrace,'String');
Settings.FlaggedTraceValue=get(handles.FlaggedTrace,'Value');
...
stimdurstrace=get(handles.TraceStimDurs,'Value');
flags=get(handles.FlaggedTrace,'Value');

stimdurtarget=str2num(get(handles.StimDurTarget,'String'));
...
% a first loop to determine the index of the last File of target stimdur
...
for z=1:length(SpikeTraceData(nbtracestrace).Trace) % loop over Files (ie groups of PSTHs from one stimulus sequence)    
    ...
    if (SpikeTraceData(stimdurstrace).Trace(k)==stimdurtarget && SpikeTraceData(flags).Trace(k)==0) 
        lastfile=z;
    end
end
...
for z=1:length(SpikeTraceData(nbtracestrace).Trace) % loop over Files (ie groups of PSTHs from one stimulus sequence)
    ...
    if (SpikeTraceData(stimdurstrace).Trace(k)==stimdurtarget && SpikeTraceData(flags).Trace(k)==0) 
        ...
        titlename=['Int:' num2str(int) '/ ' num2str(intW) ' uW'];
        ...
        if int==oldint
            ...
            if z==lastfile  % last File -> plot and save
                ...
                for n=1:length(Coords2p_Array)
                    ...
                    if CoordsSign2p_Array{n}~=0
                        plot_rect_on_image_shift(CoordsSign2p_Array{n},'r',p,xstart,ystart);
                    end
                    
                end
                
                % save fig p
                fname=[handles.Path '\' 'Int' num2str(int) '_' num2str(intW) 'uW_Dur_' num2str(stimdurtarget) 's_' SpikeTraceData(inds(1)).Filename '.fig'];
                title(['Int ' num2str(int) '/' num2str(intW) ' uW, Stim. Dur ' num2str(SpikeTraceData(stimdurstrace).Trace(k)) 's']);
                saveas(p,fname);
            end
            ...
            if oldint>0 % this condition excludes the first File
                ...
                % save fig p
                fname=[handles.Path '\' 'Int' num2str(oldint) '_' num2str(oldintW) 'uW_Dur_' num2str(stimdurtarget) 's_' SpikeTraceData(oldinds(1)).Filename '.fig'];
                title(['Int ' num2str(int) '/' num2str(intW) ' uW, Stim. Dur ' num2str(SpikeTraceData(stimdurstrace).Trace(k)) 's']);
                saveas(p,fname);
            end
            ...
            if z==lastfile  % last File -> plot and save
                ...
                % save fig p
                fname=[handles.Path '\' 'Int' num2str(int) '_' num2str(intW) 'uW_Dur_' num2str(stimdurtarget) 's_' num2str SpikeTraceData(inds(1)).Filename '.fig'];
                title(['Int ' num2str(int) '/' num2str(intW) ' uW, Stim. Dur ' num2str(SpikeTraceData(stimdurstrace).Trace(k)) 's']);
                saveas(p,fname);
            end
        end
...
function fig=plot_rect_on_image_shift(coords,scolor,fig,xshift,yshift)
...
function StimDurTarget_Callback(hObject, eventdata, handles)
% hObject    handle to StimDurTarget (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of StimDurTarget as text
%        str2double(get(hObject,'String')) returns contents of StimDurTarget as a double


% --- Executes during object creation, after setting all properties.
function StimDurTarget_CreateFcn(hObject, eventdata, handles)
% hObject    handle to StimDurTarget (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in FlaggedTrace.
function FlaggedTrace_Callback(hObject, eventdata, handles)
% hObject    handle to FlaggedTrace (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns FlaggedTrace contents as cell array
%        contents{get(hObject,'Value')} returns selected item from FlaggedTrace


% --- Executes during object creation, after setting all properties.
function FlaggedTrace_CreateFcn(hObject, eventdata, handles)
% hObject    handle to FlaggedTrace (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
