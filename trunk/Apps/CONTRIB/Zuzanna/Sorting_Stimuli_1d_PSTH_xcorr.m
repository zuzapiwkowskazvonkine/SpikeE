...
% Last Modified by GUIDE v2.5 18-Oct-2012 22:16:54
...
global DLPstim
   ...
   if ~isempty(SpikeTraceData)
        for i=1:length(SpikeTraceData)
            TextTrace_BinStims{i}=[num2str(i),' - ',SpikeTraceData(i).Label.ListText];
        end
    ...
    if ~isempty(SpikeTraceData)
        for i=1:length(SpikeTraceData)
            TextTrace_BinSpikes{i}=[num2str(i),' - ',SpikeTraceData(i).Label.ListText];
        end
...
filelabel=SpikeTraceData(1).Filename;
pathlabel=SpikeTraceData(1).Path;
Stims_array_2_psth=sort_stimuli_psth(Savedxin,Savedyin,SpikeTraceData(TracesToApply_Stims).XVector,SpikeTraceData(TracesToApply_BinStims).Trace,SpikeTraceData(TracesToApply_BinSpikes).Trace,lag,first_stim,last_stim,first_stim_in_detstim,binsize,filelabel,pathlabel,savefile_epstart,savefile_eplast,handles);
...
function Stims_array = sort_stimuli_psth(Savedxin, Savedyin, stimontimes_def, binstimtimes, binspiketimes, lag,first_stim, last_stim,first_stim_in_detstim,binsize,fname,pname,ep1,eplast,handles)
    ...
    for i=1:max_x
        for j=1:max_y
            ...
            if sz>0
                ...
                SpikeTraceData(BeginTrace).Trace=ccs/(sum(tempstims)*binsize);
                SpikeTraceData(BeginTrace).XVector=lags;
                SpikeTraceData(BeginTrace).Label.ListText=['psth' string1 string2 ' X:' int2str(i) ' Y:' int2str(j)];
                SpikeTraceData(BeginTrace).Label.YLabel='firing rate (Hz)';
                SpikeTraceData(BeginTrace).Label.XLabel='time (bins)';
                SpikeTraceData(BeginTrace).Filename=[fname '_ep' int2str(ep1) '-ep' int2str(eplast)];
                SpikeTraceData(BeginTrace).Path=pname;
