...
% Last Modified by GUIDE v2.5 21-Nov-2012 14:12:43
...
if isempty(SpikeTraceData)
 ...
 fname=SpikeTraceData(1).Filename;
 pname=SpikeTraceData(1).Path;
  
    SpikeTraceData(BeginTrace).Trace=ccs/(sum(stimtimes)*binsize);
    SpikeTraceData(BeginTrace).XVector=lags;
    SpikeTraceData(BeginTrace).Label.ListText=['psth' string1 string2 ' X:1 Y:1'];
    SpikeTraceData(BeginTrace).Label.YLabel='firing rate (Hz)';
    SpikeTraceData(BeginTrace).Label.XLabel='time (bins)';
    SpikeTraceData(BeginTrace).Filename=[fname '_ep' int2str(savefile_epstart) '-ep' int2str(savefile_eplast)];     % CORRECTED HERE
    SpikeTraceData(BeginTrace).Path=pname;            % CORRECTED HERE 
