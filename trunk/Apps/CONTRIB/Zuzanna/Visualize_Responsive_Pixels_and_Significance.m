function varargout = Visualize_Responsive_Pixels_and_Significance(varargin)
% VISUALIZE_RESPONSIVE_PIXELS_AND_SIGNIFICANCE M-file for Visualize_Responsive_Pixels_and_Significance.fig
%      VISUALIZE_RESPONSIVE_PIXELS_AND_SIGNIFICANCE, by itself, creates a new VISUALIZE_RESPONSIVE_PIXELS_AND_SIGNIFICANCE or raises the existing
%      singleton*.
%
%      H = VISUALIZE_RESPONSIVE_PIXELS_AND_SIGNIFICANCE returns the handle to a new VISUALIZE_RESPONSIVE_PIXELS_AND_SIGNIFICANCE or the handle to
%      the existing singleton*.
%
%      VISUALIZE_RESPONSIVE_PIXELS_AND_SIGNIFICANCE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VISUALIZE_RESPONSIVE_PIXELS_AND_SIGNIFICANCE.M with the given input arguments.
%
%      VISUALIZE_RESPONSIVE_PIXELS_AND_SIGNIFICANCE('Property','Value',...) creates a new VISUALIZE_RESPONSIVE_PIXELS_AND_SIGNIFICANCE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Visualize_Responsive_Pixels_and_Significance_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Visualize_Responsive_Pixels_and_Significance_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Visualize_Responsive_Pixels_and_Significance

% Last Modified by GUIDE v2.5 21-Nov-2013 17:36:46

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Visualize_Responsive_Pixels_and_Significance_OpeningFcn, ...
                   'gui_OutputFcn',  @Visualize_Responsive_Pixels_and_Significance_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Visualize_Responsive_Pixels_and_Significance is made visible.
function Visualize_Responsive_Pixels_and_Significance_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Visualize_Responsive_Pixels_and_Significance (see VARARGIN)

% Choose default command line output for Visualize_Responsive_Pixels_and_Significance
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Visualize_Responsive_Pixels_and_Significance wait for user response (see UIRESUME)
...
function varargout = Visualize_Responsive_Pixels_and_Significance_OutputFcn(hObject, eventdata, handles) 
...
SpikeTraceData(BeginTrace).XVector=1:length(handles.correctedsign);
SpikeTraceData(BeginTrace).Trace=handles.correctedsign;
SpikeTraceData(BeginTrace).DataSize=length(handles.correctedsign);

name=['corrected ' SpikeTraceData(signresptrace).Label.ListText];
SpikeTraceData(BeginTrace).Label.ListText=name;
SpikeTraceData(BeginTrace).Label.YLabel=SpikeTraceData(signresptrace).Label.YLabel;
SpikeTraceData(BeginTrace).Label.XLabel=SpikeTraceData(signresptrace).Label.XLabel;
SpikeTraceData(BeginTrace).Filename=SpikeTraceData(signresptrace).Filename;
SpikeTraceData(BeginTrace).Path=SpikeTraceData(signresptrace).Path;
...
listtosave=sort([BeginTrace-1 BeginTrace nbrespstrace xsizetrace ysizetrace intstrace intstabletrace nbtracestrace]);
...
if handles.correctedsign(inds(k)-psthtrace1+1)==1
    colorpsth='b';
else
    colorpsth='k';
end
...
% v=axis;
xmin=min(SpikeTraceData(inds(k)).XVector);
xmax=max(SpikeTraceData(inds(k)).XVector);
...
title(['Xsize:' num2str(xsize) ', Ysize:' num2str(ysize) ', Int:' num2str(int) '/ ' num2str(intW) ' uW']);
...
global SpikeTraceData
    ...
    % actually initialize not with zeros but with the uncorrected values:
    nbrespstrace=get(handles.TraceNbResps,'Value');
%     handles.corrected=zeros(1,length(SpikeTraceData(nbrespstrace).Trace));
    handles.corrected=SpikeTraceData(nbrespstrace).Trace;
    
    signresptrace=get(handles.TraceSignResp,'Value');
%     handles.correctedsign=zeros(1,length(SpikeTraceData(signresptrace).Trace));
    handles.correctedsign=SpikeTraceData(signresptrace).Trace;
...
nbtracestrace=get(handles.TraceNbTraces,'Value');
filenb=str2num(get(handles.ViewedFile,'String'));
rem=str2num(get(handles.ToRemove,'String'));
% signresptrace=get(handles.TraceSignResp,'Value');

% get index of resp to change from 1 to 0 in Trace of Significant Responses:
tottraces=sum(SpikeTraceData(nbtracestrace).Trace(1:filenb-1));
torem=tottraces+rem;

handles.correctedsign(torem)=0;
handles.corrected(filenb)=handles.corrected(filenb)-1;
...
function ToRemove_Callback(hObject, eventdata, handles)
% hObject    handle to ToRemove (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of ToRemove as text
%        str2double(get(hObject,'String')) returns contents of ToRemove as a double


% --- Executes during object creation, after setting all properties.
function ToRemove_CreateFcn(hObject, eventdata, handles)
% hObject    handle to ToRemove (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
